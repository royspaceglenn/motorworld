...
export function computeMigrationPlan(parsed) {
    ...
    destinations: [
      {
        ...
        count: sales.length,
        detail: parsed.dateRange ? `${parsed.dateRange.start} → ${parsed.dateRange.end}` : undefined,
      },
      {
        id: 'sales_summary',
      ...
      },
      {
        ...
        detail:
          receivableSales.length > 0
            ? `${cashSales.length} cash sale(s) skip receivables`
            : 'All cash — none',
      },
      {
        id: 'accounts',
